...
namespace Server.Custom.KoperPets
{
    public static class KoperPetNursery
    {
        ...
        public static void StorePet(PlayerMobile owner, BaseCreature pet)
        {
            ...
            owner.SendMessage(string.Format("You have stored {0} in your nursery.", pet.Name));
        }
        ...
        public static List<KoperStoredPet> GetStoredPets(PlayerMobile owner)
        {
            if (owner == null)
                return new List<KoperStoredPet>();

            if (!_nurseryPets.ContainsKey(owner))
                return new List<KoperStoredPet>();

            return _nurseryPets[owner]; // Return stored pets list
        }
